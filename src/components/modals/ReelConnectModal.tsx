...
export function ReelConnectModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md animate-fade-in">
      <div className="relative w-full max-w-lg rounded-3xl border border-cyan-500/30 bg-white dark:bg-[#0c1222] p-6 sm:p-7 shadow-2xl shadow-cyan-500/10 text-slate-900 dark:text-white space-y-5">
        {/* Header */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                Projet : <span className="text-cyan-700 dark:text-cyan-300 font-semibold">{projectTitle}</span>
              </p>
            </div>
          </div>
          <button
        ...
        {sentSuccess ? (
          <div className="py-8 flex flex-col items-center justify-center text-center space-y-3 animate-fade-in">
            <div className="h-12 w-12 rounded-full bg-emerald-500/20 border border-emerald-500/40 flex items-center justify-center text-emerald-500 dark:text-emerald-400">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {reasons.map((r) => (
                  <label
                    ...
                    className={`flex items-center gap-3 p-3 rounded-2xl border text-xs cursor-pointer transition ${
                      selectedReason === r.id
                        ? 'bg-cyan-50 dark:bg-cyan-950/60 border-cyan-400 dark:border-cyan-500/50 text-cyan-900 dark:text-cyan-200 shadow-sm'
                        : 'bg-slate-50 dark:bg-[#070b16] border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-700'
                    }`}
                  >
                    <input
                      type="radio"
                      ...
                      className="text-cyan-500 focus:ring-cyan-500 bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700 h-4 w-4"
                    />
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-[#070b16] p-3 text-xs sm:text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:border-cyan-500 focus:outline-none focus:ring-1 focus:ring-cyan-500 transition"
              />
            </div>
