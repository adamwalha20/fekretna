...
export function ReelAdviceModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md animate-fade-in">
      <div className="relative w-full max-w-lg rounded-3xl border border-amber-500/30 bg-[#0c1222] p-6 sm:p-7 shadow-2xl shadow-amber-500/10 text-white space-y-5">
        {/* Header */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                Projet : <span className="text-amber-300 font-semibold">{projectTitle}</span>
              </p>
            </div>
          </div>
          <button
        ...
        {/* Info banner */}
        <p className="text-xs text-slate-300 bg-amber-950/40 border border-amber-500/20 rounded-2xl p-3 leading-relaxed">
          ...
          <div className="flex flex-wrap gap-1.5">
            {quickPrompts.map((qp, idx) => (
              <button
                ...
                className="text-[11px] px-2.5 py-1 rounded-full bg-slate-900 border border-slate-800 text-slate-300 hover:border-amber-500/40 hover:text-amber-300 transition"
              >
                {qp}
              </button>
            ))}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <textarea
              ...
              className="w-full rounded-2xl border border-slate-800 bg-[#070b16] p-3.5 text-xs sm:text-sm text-slate-100 placeholder-slate-500 focus:border-amber-400 focus:outline-none focus:ring-1 focus:ring-amber-400 transition"
            />
