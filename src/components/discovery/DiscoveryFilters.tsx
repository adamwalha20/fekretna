...
export function DiscoveryFilters({
  ...
  return (
    <div className="rounded-3xl border border-slate-800/80 bg-[#0c1322]/90 p-4 sm:p-5 shadow-xl backdrop-blur-xl mb-6">
      {/* Search Input Bar */}
      <div className="relative mb-3.5">
        <Search className={`absolute top-1/2 -translate-y-1/2 h-4 w-4 text-cyan-400 ${isRtl ? 'right-3.5' : 'left-3.5'}`} />
        <input
          type="text"
          value={filters.search}
          onChange={(e) => onChange({ ...filters, search: e.target.value })}
          placeholder={t('discovery.searchPlaceholder') || 'Rechercher par nom, compétence, mot-clé...'}
          className={`w-full rounded-2xl border border-slate-800/90 bg-[#080e1e] py-2.5 text-sm text-slate-100 placeholder-slate-500 focus:border-cyan-500 focus:outline-none focus:ring-1 focus:ring-cyan-500/50 ${
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2.5">
        ...
        <div>
          ...
          <select
            ...
            className="w-full rounded-xl border border-slate-800 bg-[#080e1e] px-2.5 py-1.5 text-xs text-slate-200 focus:border-cyan-500 focus:outline-none"
          >
        ...
        {hasActiveFilters && (
          <div className="flex items-end">
            <button
              onClick={onReset}
              className="inline-flex items-center gap-1 text-xs text-rose-400 hover:text-rose-300 font-medium py-1.5 transition"
            >
