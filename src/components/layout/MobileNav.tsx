          ...
          >
            <Icon
              className={cn(
                ...
                link.active ? 'scale-110 text-cyan-600 dark:text-cyan-400 drop-shadow-sm dark:drop-shadow-[0_0_8px_rgba(6,182,212,0.6)]' : 'text-slate-500 dark:text-slate-400'
              )}
            />
            <span className="mt-0.5 tracking-tight truncate max-w-[60px]">{link.label}</span>
            {link.active && (
              <span className="absolute -top-0.5 h-1 w-1 rounded-full bg-cyan-600 dark:bg-cyan-400 shadow-[0_0_6px_#0284c7] dark:shadow-[0_0_6px_#22d3ee]" />
            )}
          </Link>
        );
      })}
